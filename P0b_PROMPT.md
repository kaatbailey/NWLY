...
> # ✅ DONE — 2026-08-30. VERDICT: COMPLETE.
> **Findings folded into `STATE.md` §16.9–§16.14. Read that, not this.**
>
> **What was achieved:** the queue response read in full. The world address is
> **`LoginQueueResponse.Token.RepAddress`**, a literal `"ip:port"` string, at frame
> 1660 — **63 frames before** the client's first DTLS ClientHello to that exact
> host:port, and present in **exactly one** of 20+ exported HTTP objects. **OPEN-1
> closed.** The mechanism this prompt proposed worked: a cold launch forced a full
> handshake (Certificate at frame 487), the keylog recorded its key, and the body
> decrypted. Predictions 1–4 **all confirmed**.
>
> **Also produced:** **OPEN-2 closed** — `GUID2` is the **queue ticket id**, not
> instance/shard/channel and not `CharacterId`; the **poll-loop discovery** (§16.11),
> which changes what S0 must answer and was very nearly this chunk's failure mode;
> `Signature` characterised as RSA-2048 and `HostHash` as an unresolved 32 bytes
> (**FIND-4**); **OPEN-3** raised as the gate on S0's design shape, owned by the new
> chunk **S0a**; four §13 correction rows and test rows **#55–#59**.
>
> **S0 is unblocked**, and with P0b done the perishable window has been spent on the
...
the response.** **This worked exactly as written — 2026-08-30. Nothing in this
section needed correcting.** If the response names the world server, prediction 1 (STATE §16.5) is
...
**The escalation ladder is UNTESTED above its third rung — 2026-08-30.** The operator
went straight to the Steam re-login and it worked first try, so the two cheaper
options were never attempted and **nothing is known about whether they suffice**.
**Not struck — unknown, not wrong.** A future capture should start cheaper and record
which rung worked; that datapoint is still missing and it costs one launch to get.
...
2. It is **JSON**, and carries the address as a literal IP + port (not a hostname —
   ...
   **Confirmed. One refinement raised in-session was wrong** — the address is a
   **single joined `"ip:port"` string**, not split across fields. §13.
...
4. `GUID2` in `{WorldId}_{GUID2}` is **echoed or explained** by the response — ~~instance,
   ...
   **Confirmed as to echo, WRONG as to meaning — corrected 2026-08-30.** It is the
   **queue ticket id** (`Token.TicketId == GUID2`, and the outer `TicketId` is
   literally `"{WorldId}_{GUID2}"`), **server-minted** in the enqueue response and
   echoed back by the client. None of instance, shard or channel. A `CharacterId`
   hypothesis raised in-session was also falsified. STATE §16.12, test #57, §13.
